...
def get_double_ratios(datadir, theta_name, delta_lmb_index=1, order=3):
    # ======================================================================
    ...
    for lambda_index, lambda_value in enumerate(lambda_list[:-delta_lmb_index]):
        lambda_value_next = lambda_list[lambda_index + delta_lmb_index]
        # print(f"\n{lambda_value_next-lambda_value=}")
        # Make the ratio at this lambda value
        gevp_correlators = data_fh[lambda_index][f"order{order}_corrs"]
        ratio_corr = np.abs(gevp_correlators[0] / gevp_correlators[1])
        # Ratio at the next lambda value
        gevp_correlators_next = data_fh[lambda_index + delta_lmb_index][
            f"order{order}_corrs"
        ]
...
def get_energydiff(datadir, theta_name, delta_lmb_index=1):
    # ======================================================================
    # Read data from the six_point.py gevp analysis script
    config = read_config(theta_name)
    defaults = read_config("defaults")
    ...
    energydiff = data_fh[0]["order3_fit"][:, 1]
    print(f"{np.shape(energydiff)=}")
    print(f"{np.average(energydiff)=}")
    return energydiff


def plot_double_ratio(double_ratio, best_fit, run_name, plotname, plotdir):
    fit_param = best_fit["param"]
    red_chi_sq = best_fit["redchisq"]
    ...
    plt.fill_between(
        ...
        linewidth=0,
        color=_colors[0],
        label=rf"$\chi^2_{{\textrm{{dof}}}} = {red_chi_sq:.2f}$",
    )
    plt.xlim(0, 22)
    plt.ylim(0, 0.013)
    # plt.ylim(0, 0.026)
...
def analyse_double_ratios_higher(
    ...
    for lambda_index, double_ratio in enumerate(
        double_ratios[: -(fit_points * lmb_step - lmb_step)]
    ):
        lambda_value = lambda_list[lambda_index]
    ...
    mat_elem_fit_2_lower = np.sqrt(
        mat_elem_fit_2**2
        - np.sqrt(np.abs(lambda4_factor)) * 2 * np.average(energydiff)
    )
    ...
    deltaE_diff = np.array(deltaE_diff)
    approx_matelem = deltaE_diff / (2 * (lambda_list[delta_lmb_index] - lambda_list[0]))

    print(len(lambda_list))
    print(len(lambda_list[: -(fit_points * lmb_step - lmb_step)]))
    print(len(mat_elem_fit))

    fig = plt.figure(figsize=(7, 5))
    fig.subplots_adjust(bottom=0.15, left=0.17)
    plt.errorbar(
        lambda_list[:-delta_lmb_index],
    ...
    plt.errorbar(
        ...
        lambda_list[: -(fit_points * lmb_step - lmb_step) - delta_lmb_index] + offset,
    ...
    plt.errorbar(
        lambda_list[: -(fit_points * lmb_step - lmb_step) - delta_lmb_index]
        + offset * 2,
    ...
    plt.errorbar(
        lambda_list[: -(fit_points * lmb_step - lmb_step) - delta_lmb_index][6]
        + offset * 2,
        np.average(mat_elem_fit_2_lower, axis=1)[6],
        np.std(mat_elem_fit_2_lower, axis=1)[6],
        capsize=4,
        elinewidth=1,
        color=_colors[4],
        fmt=_fmts[2],
        label=r"Chosen point",
    )
    plt.fill_between(
        lambda_list,
    ...
    return mat_elem_fit_2_lower, mat_elem_fit_2_upper


def fit_2ptfn_1exp(correlator, datadir, lambda_value, lmbdiff, run_name, order=3):
    ...
    datafile = datadir / Path(
        f"{run_name}/double_ratio_l{lambda_value:.7f}_dl{lmbdiff:.7f}_fitlist_1exp_order{order}.pkl"
    )
...
def main():
    ...
    energydiff = [0.0366, 0.0351, 0.0301, 0.0182, 0.0030, -0.0037]
    delta_lmb_index = 1
    lmbdiff = 0.0035714285714285718 * delta_lmb_index
    order = 3

    chosen_matrix_elements = []
    for idir, datadir_runx in enumerate(datadir_runs):
        ...
        if True:
            ...
            lambda_list, double_ratios = get_double_ratios(
                datadir_runx, theta_names[idir], delta_lmb_index, order
            )
            # # Fit to the double ratios
            # for lambda_index, lambda_value in enumerate(lambda_list[:-delta_lmb_index]):
            #     fit_2ptfn_1exp(
            #         double_ratios[lambda_index],
            #         datadir,
            #         lambda_value,
            #         lmbdiff,
            #         run_names[idir],
            ...
            energydiff_ = get_energydiff(
                datadir_runx, theta_names[idir], delta_lmb_index
            )
            ...
            for lambda_index, lambda_value in enumerate(lambda_list[:-delta_lmb_index]):
                datafile = datadir / Path(
                    f"{run_names[idir]}/double_ratio_l{lambda_value:.7f}_dl{lmbdiff:.7f}_fitlist_1exp_order{order}.pkl"
                )
                with open(datafile, "rb") as file_in:
                    fitlist = pickle.load(file_in)
                    fitlists_lambda.append(fitlist)

            mat_elem_fit_2_lower, mat_elem_fit_2_upper = analyse_double_ratios_higher(
                double_ratios,
                fitlists_lambda,
                lambda_list,
                datadir,
            ...
            chosen_matrix_elements.append(mat_elem_fit_2_lower[6])
            print("\n\n")
            print(
                err_brackets(
                    np.average(mat_elem_fit_2_lower[6]), np.std(mat_elem_fit_2_lower[6])
                )
            )

    chosen_matrix_elements = np.array(chosen_matrix_elements)
    print("\n\n")
    for mat_elem in chosen_matrix_elements:
        print(err_brackets(np.average(mat_elem), np.std(mat_elem)))
    datafile = datadir / Path(f"matrix_element_values.pkl")
    datafile.parent.mkdir(parents=True, exist_ok=True)
    with open(datafile, "wb") as file_out:
        pickle.dump(chosen_matrix_elements, file_out)
